    ...
    public partial class frmHastaDetay : Form
    {
        ...
        private void frmHastaDetay_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlCommand komut = new SqlCommand("select HastaAd, HastaSoyad from tbl_Hasta where HastaTC = @p1", bgl.baglanti()))
                {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter("select * from tbl_randevular where HastaTC =" + tc, bgl.baglanti()))
                {
        ...
        private void cmbBrans_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlCommand komut3 = new SqlCommand("select DoktorAd, DoktorSoyad from tbl_Doktor where DoktorBrans = @p1", bgl.baglanti()))
                {
        ...
        private void cmbDoktor_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter("select * from tbl_randevular where RandevuBrans = '" + cmbBrans.Text + "'" + " and RandevuDoktor = '" + cmbDoktor.Text + "' and  randevudurum = 0", bgl.baglanti()))    //Sql serverda string ifadeler ' ile yazıldığından öyle yazdık !!!
                {
                    da.Fill(dt);
        ...
        private void btnRandevuAl_Click(object sender, EventArgs e)
        {
            try
            {
                using (SqlCommand komut4 = new SqlCommand("update tbl_randevular SET randevudurum = 1,HastaTC = @r1,hastasikayet = @r2 where randevuID = @r3", bgl.baglanti()))
                {
